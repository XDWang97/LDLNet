...
parser.add_argument('--pth_path', type=str, default='/home/ubuntu/wxd/cod/code/LDLNet/ours/checkpoints/Final')
parser.add_argument('--name', type=str, default='LDLNet')
...
for pth in sorted(os.listdir(opt.pth_path)):
    ...
    for _data_name in ['CAMO','CHAMELEON','COD10K','NC4K']:
        ...
        save_path = './results/LDLNet/{}/'.format(_data_name)
        ...
        for i in range(test_loader.size):
            ...
            _,_,_,_,_,_,outb1,outd1,out1 = model(image)
            ...
            imageio.imwrite(save_path+name, (out1*255).astype(np.uint8))
            
            outb1 = F.upsample(outb1, size=gt.shape, mode='bilinear', align_corners=False)
            outb1 = outb1.sigmoid().data.cpu().numpy().squeeze()
            outb1 = (outb1 - outb1.min()) / (outb1.max() - outb1.min() + 1e-8)
            imageio.imwrite(save_path+'body/'+name, (outb1*255).astype(np.uint8))
            
            outd1 = F.upsample(outd1, size=gt.shape, mode='bilinear', align_corners=False)
            outd1 = outd1.sigmoid().data.cpu().numpy().squeeze()
            outd1 = (outd1 - outd1.min()) / (outd1.max() - outd1.min() + 1e-8)
            imageio.imwrite(save_path+'detail/'+name, (outd1*255).astype(np.uint8))
        
    file.write(pth+"\n")
    method='LDLNet'
    for _data_name in ['CAMO','CHAMELEON','COD10K','NC4K']:
        mask_root = './data/TestDataset/{}/GT'.format(_data_name)
        pred_root = './results/LDLNet/{}/'.format(_data_name)
